...
$(function($){
    	...
    	function simpleQuery()
    	{
    		//$("#btnClear").click();
    		clearMoreSearchForm();
    		$("#proCode").val($("#proCodeSearch").val());
    		$("#proName").val($("#proNameSearch").val());
    		$("#orgId").val($("#orgIdSearch").val());
    		$("#pageNo").val('1');
    	...
    	function deleteData()
    	{
    		...
    		layer.confirm('确认要删除"'+dt+'"？', {
    		  btn: ['确定','取消'] //按钮
    			...
    			$.ajax({
    				   type: "POST",
    				   url: path+"/projectDelete.htm?proId="+pid,
    	...
    	function loadEdit()
    	{
    		...
    		if($(this).attr("pid"))
    			{
    			  url=path+"/projectLoadUpdate.htm?proId="+$(this).attr("pid");
    			}
    		else
    			{
    			url=path+"/projectLoadAdd.htm?"
    			}
