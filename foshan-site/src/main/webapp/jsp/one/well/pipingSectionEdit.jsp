...
<body>
    ...
    <form id="addform" name="addform" action="${path}/pipingSectionUpdateSave.htm" method="post">
    <ul class="forminfo">
     <li><label>管道段名称：</label><input id="pipingName" name="pipingName" type="text" class="dfinput" onBlur="userNameVerify()" value="${pb.pipingName}"/><font style="font-size: 25px" color=red>*</font></li>
     <li><label>管道段状态: </label>
    <div class="vocation">
	    <select class="select3" name="pipingSectType" id="pipingSectType" onchange="changeRank();">
	    <option value='${pb.pipingSectType}' selected="selected"></option>
    ...
    <div class="vocation">
	    ...
	    <c:if test="${pb.isImport == '0'}">
	    <option value=''>=请选择=</option>
	    <option value='0' selected="selected">否</option>
	    <option value='1'>是</option>
	    </c:if>
	     <c:if test="${pb.isImport == '1'}">
	    <option value=''>=请选择=</option>
    ...
    <li><label>物理长度：</label><input id="phyLen" name="phyLen" type="text" class="dfinput" onkeyup="value=value.replace(/[^\w\.\/]/ig,'')" value="${pb.phyLen }"/><font style="font-size: 25px" color=red>*</font></li> 
    <li><label>地图长度：</label><input id="mapLen" name="mapLen" type="text" class="dfinput" onkeyup="value=value.replace(/[^\w\.\/]/ig,'')" value="${pb.mapLen }"/><font style="font-size: 25px" color=red>*</font></li> 
    <li><label>&nbsp;</label><input name="directFlg" type="checkbox" value="1" />保存并继续录入&nbsp;
    <input name="" type="submit" class="btn" value="确认保存" onclick="checkValue();"/></li>
    </ul>
    </form>
    </div>
