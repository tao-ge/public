 ...
 <!-- 页面导航  -->
	<div class="place">
	    ...
	    <ul class="placeul">
		    ...
		    <li><a href="devStatusList.htm">设备实时监控</a></li>
	    </ul>
    </div>
	...
	function aa(){
		...
		if(areaName != null){
			...
			if(p0.length>1){//超过两个，用市
				map.centerAndZoom(centerName, 13);
			}else{//只有一个，用当前名字
		...
		if (document.createElement('canvas').getContext) { // 
			...
			$.ajax({
				type : "post",
				url : path + "/selectDeviceRegAreaCode1.htm?devName="+devName+"&areaCode1="+areacode,
				dataType : "json",
				success : function(data) {
					var isStatus = data.isStatus;
					var isWorkStatus = data.isWorkStatus;
					if(isStatus == '1'){//报警 红色
					...
					for (var i = 0; i < data.length; i++) {
						var point = new BMap.Point(data[i].baiduX,
								data[i].baiduY);
						var type = data[i].devStatus;
						var devId =  data[i].devId;
						var devName = data[i].fdevName;
						var typeName = null;
						if (type == "0") {
							typeName = "正常";
						...
						var title = "";
						title = "设施名称：" + data[i].fdevName + 
								"<br>设施编码："+ data[i].devCode + 
								"<br>工作状态："+ typeName + 
								"<br>经度：" + data[i].longitude +
								"<br>纬度：" + data[i].latitude;
						...
						marker.setLabel(label);
						 ...
						 function addClickHandler(devId,devName){//点击设施查询下方表格
				        	marker.addEventListener("click",function(e){
					        		$('#fff').attr('href',path+"/queryDeviceAlarm.htm?devName="+devName+"&devId="+devId); 
									$("#device").trigger("click");
				    			}
				    		);
					    } 
